  ...
  return (
    <div className="mx-auto flex max-w-md flex-col items-center rounded-2xl border border-dashed border-paper-line bg-white px-8 py-16 text-center">
      ...
      <p className="mt-2 text-sm text-ink-soft">
        {query ? (
          <>
            Nothing matched &ldquo;{query}&rdquo; in our current listings — but don't
            worry, we may still be able to help.
          </>
        ) : (
          <>No events match your filters right now — but we may still be able to help.</>
        )}
      </p>
      <Link
        to={requestHref}
