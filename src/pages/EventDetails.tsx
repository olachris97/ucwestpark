          ...
          <div>
            ...
            <div className="mt-10 rounded-xl border border-paper-line bg-paper-stub/40 p-6">
              <h3 className="font-display text-lg font-semibold text-ink">
                How parking requests work
              </h3>
              <p className="mt-2 text-sm leading-relaxed text-ink-soft">
                Submitting a request does not charge you. Our team reviews available
                options that match your number of spots, preferred location, and budget, then follows up
...
function TicketRequestPanel({
  ...
  if (submitted) {
    return (
      <aside className="h-fit rounded-2xl border border-paper-line bg-white p-7 lg:sticky lg:top-24">
        ...
        <p className="mt-2 text-sm leading-relaxed text-ink-soft">
          Thank you! We've received your parking request for{" "}
  ...
  return (
    <aside className="h-fit rounded-2xl border border-paper-line bg-white p-7 lg:sticky lg:top-24">
      <h3 className="font-display text-2xl font-semibold text-ink">Request parking</h3>
      <p className="mt-1 text-sm text-ink-soft">
        {event.startingPrice ? `From $${event.startingPrice} per parking pass` : "Parking price available on request"}
      </p>
      <p className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-paper-stub px-3 py-1 text-xs font-semibold text-black">
        {event.ticketsAvailable > 0
          ? `${event.ticketsAvailable} parking passes available`
          : "Currently sourcing parking"}
      </p>

      <form className="mt-6 space-y-5" onSubmit={handleSubmit} noValidate>
        <HoneypotField value={website} onChange={setWebsite} />
        <div>
          <label className="text-sm font-medium text-ink">Number of parking passes</label>
          <div className="mt-2 flex w-fit items-center gap-4 rounded-full border border-paper-line px-2 py-1.5">
            <button
              ...
              aria-label="Decrease parking pass quantity"
            >
              <Minus size={14} />
            </button>
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-paper-stub text-ink hover:bg-gold hover:text-marquee"
              aria-label="Increase parking pass quantity"
            >
              <Plus size={14} />
            </button>
          </div>
        ...
        <div>
          <label className="text-sm font-medium text-ink">Parking preference</label>
          <div className="mt-2 space-y-2">
            {[
              { v: "any", l: "Any nearby option" },
              { v: "best", l: "Best value / closest" },
              { v: "specific", l: "Specific parking area" },
            ].map((opt) => (
          ...
          {preference === "specific" && (
            <input
              ...
              placeholder="e.g. Near Gate 2, closest entrance, easy exit"
        ...
        >
          {submitting ? "Submitting…" : "Request Parking"}
        </button>
      </form>
    </aside>
  );
